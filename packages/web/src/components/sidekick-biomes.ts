...
// Each biome is a self-contained look: a ground the character stands on, scattered
// props, distant silhouettes, plus a light/sky/fog preset so the whole scene reads
// as that place. Same technique as sidekick-landscape.ts — deterministic (no
// Math.random) so renders are stable — and everything is flat-shaded chunky geo.
...
function buildTropical(): THREE.Group {
	...
	const water = new THREE.Mesh(
		...
		new THREE.MeshStandardMaterial({ color: "#2fa8c8", roughness: 0.35, metalness: 0.1, emissive: "#1f88b0", emissiveIntensity: 0.25, fog: false }),
	);
	...
	scatter(group, 8, 60, 90, 99, (i) => {
		...
		const sand = new THREE.Mesh(new THREE.SphereGeometry(4 + rand(i) * 3, 10, 6), new THREE.MeshLambertMaterial({ color: "#e6c98a", flatShading: true }));
		sand.scale.y = 0.18;
		sand.position.y = -0.3;
		island.add(sand);
...
function buildVolcano(): THREE.Group {
	...
	const smokeMat = new THREE.MeshLambertMaterial({ color: "#6a5f5f", transparent: true, opacity: 0.7, flatShading: true, fog: false });
	for (let i = 0; i < 5; i++) {
