...
//
// The catalog under the top-level assets/cosmetics/ is the canonical art source
// — the Blender char-pipeline (tools/char-pipeline) writes there. See ASSETS below.
//
...
const ASSETS = join(EXPO, '..', '..', 'assets'); // top-level canonical art source
const COS_SRC = join(ASSETS, 'cosmetics');
...
for (const [id, def] of Object.entries(manifest)) {
  ...
  // (byte-identical to assets/cosmetics/phone/base-v1.glb) so we don't touch it.
  let modelRef;
  if (id === 'phone') {
...
const RENDER_SRC = join(ASSETS, 'shop-renders');
...
ensureDir(PROPS_DST);
const lootSrc = join(ASSETS, 'props', 'lootbox-v1.glb');
...
// assets/shop-renders) is exposed as a renderKey → module-ref map.
