// Sync the web app's cosmetics catalog into the Expo app's bundled assets.
//
// The web app (packages/web) is the reference: it fetches
// public/cosmetics/manifest.json at runtime and loads .webp textures + GLBs by
// URL. Metro can't do either — bundled assets must be static require() calls and
// expo-gl can't decode .webp — so we mirror the catalog into packages/expo:
...
//
// Re-run any time the web catalog changes:  node scripts/sync-cosmetics.mjs
//
...
// module instead. Structure mirrors public/cosmetics/manifest.json — model/tex
// are Metro module refs instead of URL strings, and variant textures are PNG
// (converted from the web's .webp; expo-gl can't decode .webp). The phone slot
// reuses the already-bundled stripped mascot phone GLB.
