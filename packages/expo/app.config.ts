...
// Every target in targets/ — NotificationService included — is generated by
// @bacons/apple-targets, which only runs as part of react-native-device-activity's
// config plugin. With device activity off that plugin never runs, so none of the
// targets exist in project.pbxproj and declaring any of them here fails the build
// late, in CONFIGURE_XCODE_PROJECT, with "Could not find target in project.pbxproj".
// The only cost is notification thread-grouping.
...
const getPlugins = () => {
  ...
  // The plugin's only job is uploading source maps and debug symbols, which
  // needs SENTRY_AUTH_TOKEN. Without one, sentry-cli fails the "Bundle React
  // Native code and images" phase and takes the whole archive down with it, so
  // a local release build is impossible unless the token is exported. Crash
  // reporting itself is unaffected — that runs off EXPO_PUBLIC_SENTRY_DSN in
  // src/lib/sentry.tsx. EAS sets the token in every environment, so uploads
  // still happen for real builds.
