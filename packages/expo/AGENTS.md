...
- All features land here or in `@sidekick/core` (platform-agnostic logic —
  keep it free of DOM/RN/expo imports).
- `packages/web` (Vite) is a DEPRECATED reference only — read it when porting
  remaining behavior, never develop in it, never keep the two in sync by hand.
