...
export type SidekickController = {
  ...
  // onboarding cinematic: jump-into-frame entrance, camera shake, live body
...
export function createSidekickRenderer(
  ...
  let landed = false;
  let camShake: { start: number; dur: number; amp: number; mode: 'impact' | 'build' } | null = null;
  ...
  const animate = () => {
    ...
    if (jump) {
      ...
      if (p >= 0.9 && !landed) {
        landed = true;
        camShake = { start: now, dur: 0.6, amp: 0.42, mode: 'impact' };
      }
      if (p >= 1) {
        entranceY = 0;
        jump = null;
      ...
      entranceY = hiddenPark ? HIDDEN_Y : 0;
    }
    ...
    camera.lookAt(camBaseTarget);
    // onboarding camera shake: 'build' ramps up (suspense), 'impact' decays
    if (camShake) {
      const sp = (now - camShake.start) / camShake.dur;
      if (sp >= 1) camShake = null;
      else {
        const envl = camShake.mode === 'build' ? sp : 1 - sp;
        const a = camShake.amp * envl;
        camera.position.x += Math.sin(now * 63) * 0.5 * a;
        camera.position.y += Math.sin(now * 57) * 0.5 * a;
        camera.lookAt(camBaseTarget);
      }
    }
